...
(function ($) {
  $.fn.splitText = function (options) {
    ...
    var TMax =
      options.useLite == false ? new TimelineMax() : new TimelineLite();
    ...
    if (document.getElementById(parentID) == undefined) {
      $("body").append(
        '<p class="hiddenText" id="hidden_' + element.attr("id") + '"></p>'
      );
    ...
    function splitLetters(userInput) {
      ...
      for (var i = 0; i < arr.length; i++) {
        if (arr[i] == " ") {
          ...
          if (!arr[i].match(/\s\n\t\r/g) && arr[i] != "")
            arr[i] = '<div class="letter-measure">' + arr[i] + "</div>";
        }
      }
    ...
    function splitWords(userInput, justSplit) {
      ...
      $.each(a, function (i, val) {
        if (!val.match(/\n/) && val != "")
          a[i] = '<div class="word-measure">' + val + "</div>";
      });
    ...
    function splitLines(userInput) {
      ...
      for (var i = 1; i < count; i++) {
        var prevY = element
          .children(".word-measure:eq(" + (i - 1) + ")")
          .offset().top;
        if (
          element.children(".word-measure:eq(" + i + ")").offset().top == prevY
        ) {
    ...
    function splitSentences(userInput) {
      ...
      for (var j = 0; j < words.length; j++) {
        ...
        if (isSentenceEnd) {
          words[j] =
            "<div class='split-sentences endOfSentence'>" + word + "</div>";
        } else {
    ...
    this.animate = function () {
      if (options.animation == "glowOnHover") {
        ...
        element.children().each(function (index, value) {
          ...
          var turnangle =
            (Math.atan2(y - getRandom(100, 200), x - getRandom(100, 200)) *
              180) /
              Math.PI +
            90;
        ...
        element.children().each(function (index, value) {
          ...
          TMax.insert(
            TweenMax.to($(this), getRandom(0.5, 2.5), getMatrixTo(pos.top))
          );
        });
      ...
      if (options.type == "letters") {
        ...
        for (var i = 0; i < nChildren; i++) {
          ...
          TMax.insert(
            TweenMax.to(item, options.duration, getAnimation(options))
          );
        }
        ...
        $(".split-lines").each(function () {
          ...
          if (options.animation == "slide") {
            ...
            TMax.insert(
              TweenMax.to($(this), options.duration, getAnimation(options, w)),
              "-=0.45"
            );
          } else {
            var pos = $(this).offset();
